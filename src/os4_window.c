...
{
    if (!_glfw.vk.KHR_surface || !_glfw.vk.AMIGA_surface)
        return;

    extensions[0] = "VK_KHR_surface";
    extensions[1] = "VK_AMIGA_surface";
}
...
{
    PFN_vkGetPhysicalDeviceAmigaPresentationSupportAMIGA
        vkGetPhysicalDeviceAmigaPresentationSupportAMIGA =
        (PFN_vkGetPhysicalDeviceAmigaPresentationSupportAMIGA)
        vkGetInstanceProcAddr(instance,
            "vkGetPhysicalDeviceAmigaPresentationSupportAMIGA");

    if (!vkGetPhysicalDeviceAmigaPresentationSupportAMIGA)
    {
        _glfwInputError(GLFW_API_UNAVAILABLE,
            "AmigaOS4: Vulkan instance missing VK_AMIGA_surface extension");
        return GLFW_FALSE;
    }

    return vkGetPhysicalDeviceAmigaPresentationSupportAMIGA(
        device, queuefamily, NULL);
}
...
{
    VkResult err;
    struct VkAmigaSurfaceCreateInfoAMIGA sci;
    PFN_vkCreateAmigaSurfaceAMIGA vkCreateAmigaSurfaceAMIGA;

    vkCreateAmigaSurfaceAMIGA = (PFN_vkCreateAmigaSurfaceAMIGA)
        vkGetInstanceProcAddr(instance, "vkCreateAmigaSurfaceAMIGA");

    if (!vkCreateAmigaSurfaceAMIGA)
    {
        _glfwInputError(GLFW_API_UNAVAILABLE,
            "AmigaOS4: Vulkan instance missing VK_AMIGA_surface extension");
    ...
    sci.sType   = VK_STRUCTURE_TYPE_AMIGA_SURFACE_CREATE_INFO_AMIGA;
    sci.pScreen = window->os4.screen;
    sci.pWindow = window->os4.handle;

    err = vkCreateAmigaSurfaceAMIGA(instance, &sci, allocator, surface);
    if (err)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
            "AmigaOS4: Failed to create Vulkan surface: %s",
