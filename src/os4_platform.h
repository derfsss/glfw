...
/*
 * VulkanOS4 VK_AMIGA_surface types
 *
 * Defined here using forward declarations compatible with GLFW's own
 * minimal Vulkan type stubs in internal.h. We cannot include vulkan.h
 * or vulkan_amiga.h directly because they conflict with GLFW's typedefs.
 */

#define VK_AMIGA_SURFACE_EXTENSION_NAME  "VK_AMIGA_surface"

/* sType value — matches VK_STRUCTURE_TYPE_AMIGA_SURFACE_CREATE_INFO in vulkan_core.h */
#define VK_STRUCTURE_TYPE_AMIGA_SURFACE_CREATE_INFO_AMIGA 1000400000

/* Forward declarations — full types come from internal.h */
struct VkAmigaSurfaceCreateInfoAMIGA {
    int                     sType;      /* VkStructureType */
    const void*             pNext;
    uint32_t                flags;      /* VkAmigaSurfaceCreateFlagsAMIGA */
    struct Screen*          pScreen;
    struct Window*          pWindow;
};

typedef int (*PFN_vkCreateAmigaSurfaceAMIGA)(
    void* /* VkInstance */, const struct VkAmigaSurfaceCreateInfoAMIGA*,
    const void* /* VkAllocationCallbacks* */, uint64_t* /* VkSurfaceKHR* */);

typedef uint32_t (*PFN_vkGetPhysicalDeviceAmigaPresentationSupportAMIGA)(
    void* /* VkPhysicalDevice */, uint32_t, struct Screen*);
